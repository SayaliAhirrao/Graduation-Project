...
if(isset($_POST['update_data']))
{
 ...
 $status=$_POST['status'];
 $query="update appointmenttable set payment='$status' where contact='$contact';";
 $result=mysqli_query($con,$query);
 if($result)
...
function display_specs() {
  global $con;
  $query="select distinct(spec) from doctortable";
  $result=mysqli_query($con,$query);
  while($row=mysqli_fetch_array($result))
  {
    $spec=$row['spec'];
...
function display_age() {
    global $con;
    $query="select distinct(age) from appointmenttable";
    $result=mysqli_query($con,$query);
    while($row=mysqli_fetch_array($result))
    {
        $age=$row['age'];
        echo '<option data-value="'.$age.'">'.$age.'</option>';
    }
}

function display_gender() {
    global $con;
    $query="select distinct(gender) from appointmenttable";
    $result=mysqli_query($con,$query);
    while($row=mysqli_fetch_array($result))
    {
        $gender=$row['gender'];
        echo '<option data-value="'.$gender.'">'.$gender.'</option>';
    }
}
...
{
 $username=$_POST['username'];
 $query="insert into doctortable(username)values('$username')";
 $result=mysqli_query($con,$query);
 if($result)
